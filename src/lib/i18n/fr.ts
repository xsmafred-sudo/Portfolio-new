export const fr = {
  nav: {
    home: 'Accueil',
    services: 'Services',
    about: 'À propos',
    experience: 'Expérience',
    projects: 'Projets',
    contact: 'Contact',
  },
  hero: {
    greeting: 'Bonjour, je suis',
    title: 'Développeur Full-Stack',
    name: 'Mouil Prosper',
    subtitle:
      'Plus de 4 ans à construire des SaaS cloud-natifs sécurisés. Réduit la latence PostgreSQL de 40% pour un ensemble de données de 3M de lignes; a construit une passerelle de crypto (2M$ par mois, <0.2% de fraude); a dirigé une équipe Agile de 4 ingénieurs. Stack: Java 17, Spring Boot, Angular, React, TypeScript, AWS, GCP, Docker, CI/CD.',
    location: 'Yaoundé, Cameroun | +237 691-958-707',
    available: 'Disponible pour travailler!',
  },
  services: {
    title: 'Services',
    fullstack: {
      title: 'Développement Full-Stack',
      description:
        "Construction d'applications sécurisées et évolutives avec Java, Spring Boot, React, Next.js et TypeScript.",
    },
    cloud: {
      title: 'Cloud & DevOps',
      description:
        "Déploiement et gestion d'applications sur AWS, GCP, Docker, Vercel, et mise en œuvre de pipelines CI/CD.",
    },
    api: {
      title: "Développement d'API",
      description:
        "Conception d'API RESTful, points de terminaison GraphQL et microservices avec authentification sécurisée.",
    },
    database: {
      title: 'Gestion de Base de Données',
      description:
        'Gestion de PostgreSQL, MySQL, MongoDB et Supabase avec optimisation pour la performance et la sécurité.',
    },
    security: {
      title: 'Sécurité & Anti-Fraude',
      description:
        'Mise en œuvre des pratiques de sécurité OWASP Top-10, SSL/TLS, renforcement du réseau et détection de fraude par ML.',
    },
    uiux: {
      title: 'Développement UI/UX',
      description:
        "Création d'interfaces intuitives et responsives avec React, Angular et principes de conception modernes.",
    },
  },
  about: {
    title: 'À propos de moi',
    summary:
      'Plus de 4 ans à construire des SaaS cloud-natifs sécurisés. Réduit la latence PostgreSQL de 40% pour un ensemble de données de 3M de lignes; a construit une passerelle de crypto (2M par mois, <0.2% de fraude); a dirigé une équipe Agile de 4 ingénieurs.',
    coreTechnologies:
      'Java 17, Spring Boot, Angular, React, TypeScript, AWS, GCP, Docker, CI/CD, PostgreSQL, MySQL, MongoDB, Supabase, REST, JSON, JWT, OAuth2, WebSocket, gRPC, GraphQL, OWASP Top-10, SSL/TLS, renforcement du réseau, anti-fraude ML',
    cloudDevOps: 'AWS, GCP, Docker, Vercel, Coolify, GitHub Actions, Terraform',
  },
  experience: {
    title: 'Expérience',
    subtitle: 'Expérience professionnelle de Mouil Prosper.',
    stevo: {
      role: 'Développeur Full-Stack',
      company: 'Stevo Digital',
      period: '2023 – 2025',
      description:
        "Livré 5 fonctionnalités majeures par sprint de 6 semaines (Spring Boot + Angular/React). Réduit le temps de déploiement de 35% en migrant les charges de travail GCP vers Coolify CI/CD. Construit des micro-services REST servant plus de 1M d'appels mensuels à 99.9% de disponibilité. A augmenté l'engagement utilisateur de 18% via des itérations UI/UX basées sur Hotjar.",
    },
    microqq: {
      role: 'Ingénieur Logiciel & Support IT',
      company: 'Micro QQ Tech',
      period: '2022 – 2023',
      description:
        'Conçu un CMS Laravel pour les dossiers de maternité; accéléré la récupération des données de 40%. Administré Ubuntu VPS & PostgreSQL; zéro incident de sécurité. A mentoré 4 devs juniors sur Git flow, revues de code, tests unitaires. Déployé des mises à niveau campus-wide pour plus de 75 postes de travail.',
    },
  },
  projects: {
    title: 'Projets',
    subtitle: 'Projets sélectionnés de Mouil Prosper.',
    viewCode: 'Voir le Code',
    liveDemo: 'Démo en Direct',
  },
  contact: {
    title: 'Restons en Contact',
    subtitle:
      "Je suis actuellement à la recherche de nouvelles opportunités. Que vous ayez une question ou simplement envie de dire bonjour, j'essaierai de vous répondre au plus vite!",
    email: "M'envoyer un Email",
  },
  footer: {
    rights: 'Tous droits réservés.',
  },
};
